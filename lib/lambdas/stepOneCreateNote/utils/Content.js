...
const Content = {
  async getTemplate(mam, event) {
    ...
    <b>${homeTeamName}</b> y <b>${awayTeamName}</b> se enfrentarán este (fecha completa), desde las ${scheduledStart}, por <b>${competition}</b>. El encuentro se disputará en el estadio ${stadium}. A continuación, conocé las opciones para <b>ver en vivo el encuentro por televisión y de manera online</b>.
    </p>`;
    ...
    let partOne = first_paragraph + first_h2 + second_paragraph;
    ...
    let body = [];
    body[0] = await Content.getBodyItem(0, 'texto', partOne, null, null);
    body[1] = await Content.getBodyItem(1, 'mam', null, null, thirdPart);
    body[2] = await Content.getBodyItem(2, 'texto', secondPart, null, null);
  ...
  async createContent(mam, event) {
    ...
    const resp = {
      ...
      article: {
        not_cuerpo: JSON.stringify(stepInfo.txtBody),
      },
      terms: terms,
      match: {
        match_publish: 0,
